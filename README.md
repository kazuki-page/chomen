...
実家で所有する賃貸マンションの管理業務を対象とした Web アプリケーション（PWA）。
...
| [技術選定](docs/tech-stack.md) | 各技術の役割・選定理由・代替案・弱点 |
...
Phase 2 まで実装済み。本番環境で稼働しており、Notion からのデータ移行を進めている。
...
| 設備台帳（実施履歴から現在の設備を導出） | ✅ Phase 2 |
| 帳票印刷（入居状況・台帳・設備） | ✅ Phase 2 |
| CSV の一括登録・書き出し | ✅ Phase 2 |
| PWA（ホーム画面へのインストール） | ✅ |
| 修繕写真・請求書の添付 | 未（R2 未導入） |
| 更新期限の Web Push 通知 | 未（Phase 3） |
| 変更履歴 | 未（Phase 3） |
