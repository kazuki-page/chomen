/**
 * 開発・デモ用のシードデータを SQL 文の配列として組み立てる。
 *
 * 2箇所から呼ばれる:
 *   - `npm run db:seed` … ローカルの D1 に流す（scripts/generate-seed.mjs 経由）
 *   - デモ環境の日次リセット … workers/app.ts の scheduled ハンドラ
 *
...
/**
 * ローカル開発の基準日。決め打ちにして毎回同じデータが出るようにする。
 * デモの日次リセットは実行時の日付を渡すので、「3日後に更新予定」といった
 * 相対的な日付が時間とともに古びない。
 */
const DEFAULT_TODAY = new Date("2026-08-01T00:00:00Z");
...
export type SeedOptions = {
  ...
  /**
   * 投入先の組織 ID。
   * デモ環境では最初のアカウントを作った時点で組織が自動生成されるため、
   * 呼び出し側が実際の ID を渡す。
   */
...
/**
 * 業務データを消してシードを入れ直す SQL 文を返す。
 *
 * 返り値は1文につき1要素。`db.batch()` にそのまま渡せるよう、
 * トランザクション制御や PRAGMA は含めない。
 * 削除は子から親の順に並んでいるので、外部キーは壊れない。
 */
export function buildSeedStatements(options: SeedOptions = {}): string[] {
  const TODAY = options.today ?? DEFAULT_TODAY;
  const ORG_ID = options.organizationId ?? DEFAULT_ORG_ID;
  const NOW_MS = TODAY.getTime();

  const lines: string[] = [];
  const out = (sql: string) => lines.push(sql);

  const q = (v: Value | undefined): string => {
  ...
  const shiftDays = (days: number) => isoDate(new Date(NOW_MS + days * 86_400_000));
  ...
  out(
    `INSERT INTO organizations (\`id\`, \`name\`, \`created_at\`, \`updated_at\`) VALUES (${q(ORG_ID)}, ${q("デモ管理")}, ${NOW_MS}, ${NOW_MS}) ON CONFLICT(\`id\`) DO UPDATE SET \`updated_at\` = excluded.\`updated_at\`;`,
  );
  insert("buildings", {
    id: BUILDING_ID,
  ...
  // -------------------------------------------------------------- 部屋・駐車場
  const unitDefs: { id: string; type: string; code: string; order: number }[] = [];
  let order = 0;
  for (const floor of [1, 2, 3, 4]) {
    for (let n = 1; n <= 10; n++) {
      const code = `${floor}${String(n).padStart(2, "0")}`;
      unitDefs.push({ id: `unit_${code}`, type: "room", code, order: order++ });
    }
  }
  for (let n = 1; n <= 5; n++) {
  ...
  const procedureSeeds: {
    ...
    // 存在しない部屋。下の guard で読み飛ばされる
    { unit: "unit_112", type: "renewal", items: RENEWAL_ITEMS, checked: 0, scheduled: shiftDays(20) },
  ...
  const equipmentSeeds: [string, string, string, string | null, string | null, number][] = [
  ...
  const workOrderSeeds = [
    {
      id: "wo_1",
      unit_id: "unit_102" as string | null,
      title: "給湯器の不具合",
      description: "お湯が出ないと管理会社経由で連絡あり",
      handler: "vendor",
      waiting_on: "業者の見積" as string | null,
      status: "in_progress",
