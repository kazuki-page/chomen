...
- 当面は1棟だが、複数棟を前提とした構造にしておく（Phase 3 で UI 対応）
...
| **入居手続きの完了** | Lease を `active` に。Unit の `listing_*` をクリア。**2年後の更新手続きを自動生成** |
...
- **初期に全件登録しない。** 40室 × 設備数の一括入力は必ず破綻するため、**修繕案件の登録時に未登録の設備をその場で作成できる導線**を用意し、対応したものから台帳が育つ形にする
