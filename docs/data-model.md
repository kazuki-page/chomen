...
契約の一括登録は1行ごとにLeaseと初回家賃（`initial`）を作るため、更新の行を
別契約として取り込むと、同じ入居者の家賃履歴が分断される。
`database/repositories/lease-history-repair.server.ts` は本人による継続入居の確認後、
同じ部屋・氏名（両方に生年があれば一致必須）の直前の終了契約を現在の契約にまとめる。
過去契約に手続きがある、家賃未確定、日付が重なる場合は拒否する。
過去の家賃が未登録（改定レコードなし）の場合は統合を許可し、不明な金額を補完しない。
家賃の金額・適用日・確定状態を保持して付け替え、現在の契約開始日の`initial`を
`renewal`に直す。契約開始日を過去契約日に戻し、元の契約行だけを削除する。
現在の手続きと次回更新日は保持する。入居者行は他契約への影響を避けて保持する。
処理はD1のバッチ内で再検証・更新し、途中の失敗は全体をロールバックする。
